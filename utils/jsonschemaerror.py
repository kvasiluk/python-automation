...
from api_tests.common.exceptions import JsonValidationError
...
def check_json(json_object, schema, context=None):
    try:
        ...
        if context:
            raise JsonValidationError("JSON Schema check failed for '{}'\n{}".format(context, report))
        raise JsonValidationError("JSON Schema check failed.\n{}".format(report))
